...
initial_state["mask"] = np.zeros(shape=(n_sources_max, 1), dtype=bool)
initial_state["mask"][:initial_state["n_src"]] = 1.0
initial_state["mask"] = jnp.array(initial_state["mask"])
...
# set up the samplers
sampler = [ManifoldMALA("z", mdl, jit_compile=True),
           NormalNormal("s", mdl)]
sampler[0].max_variable_size = (3, n_sources_max)
sampler[0].step = np.array([[0.25]])

# jit compile the data mean function
data_mean.update_prefactors_jit = jax.jit(data_mean.update_prefactors)
...
def birth_function_jax(current_state: dict, prop_state: dict):
    ...
    prop_state = data_mean.update_prefactors_jit(prop_state)
